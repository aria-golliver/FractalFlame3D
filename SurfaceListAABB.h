...
class SurfaceListAABB { 
public:
	SurfaceListAABB *LHS, *RHS;
	Surface *LHS_S, *RHS_S;
	bool left_leaf, right_leaf;
	AABBox AABB;


    SurfaceListAABB(vector<Surface *> *surfaces){
		...
		if (size == 0){
			...
			vector<Surface *> surfaces_lhs;
			vector<Surface *> surfaces_rhs;
			...
			while (no_split_found) {
				...
				for (int i = 0; i < size; i++){
					mean += surfaces->at(i)->min_point[dir];
				}
				
				mean /= size;

				for (int i = 0; i < size; i++){
					if (surfaces->at(i)->min_point[dir] < mean)
			...
			if (surfaces_lhs.size() == 1){
				...
				min_z = min2(min_z, LHS_S->min_point[1]);

				max_x = max2(max_x, LHS_S->max_point[0]);
				max_y = max2(max_y, LHS_S->max_point[1]);
				max_z = max2(max_z, LHS_S->max_point[1]);
			}
			else {
				LHS = new SurfaceListAABB(&surfaces_lhs);
			...
			if (surfaces_rhs.size() == 1){
				...
				min_z = min2(min_z, RHS_S->min_point[1]);

				max_x = max2(max_x, RHS_S->max_point[0]);
				max_y = max2(max_y, RHS_S->max_point[1]);
				max_z = max2(max_z, RHS_S->max_point[1]);
			}
			else {
				RHS = new SurfaceListAABB(&surfaces_rhs);
    ...
    Surface * intersectScene(const Ray &ray, HitVector &hit) const {
		Surface *nearestSurface_lhs = NULL;
		Surface *nearestSurface_rhs = NULL;
